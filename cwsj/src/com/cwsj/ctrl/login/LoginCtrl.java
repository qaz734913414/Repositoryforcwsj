...
public class LoginCtrl extends BaseController {
	...
	@Resource(name="loginBLH")
	private LoginBLH loginBLH;
		...
		if(null != KEYDATA && KEYDATA.length == 3){
			...
			if(null == code || "".equals(code)){
				...
				if(Tools.notEmpty(sessionCode) && sessionCode.equalsIgnoreCase(code)){
					...
					pd = loginBLH.getUserByNameAndPwd(pd);
					if(pd != null){
...
//						userBLH.updateLastLogin(pd);
						...
						//开始封装角色代码
						List<Map> list=loginBLH.listRoleByUserId(pd.getString("USER_ID"));
...
//				if(null == userr){
//					user = loginBLH.getUserAndRoleById(user.getUSER_ID());
...
//				session.setAttribute(Const.SESSION_ROLE_RIGHTS, roleRights); 		//将角色权限存入session
				...
				if(null == session.getAttribute(Const.SESSION_allmenuList)){
					allmenuList = loginBLH.listAllMenu();
